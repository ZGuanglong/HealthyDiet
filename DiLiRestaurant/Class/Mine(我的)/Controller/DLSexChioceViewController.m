...
- (void)done{
    ...
    if (self.selectbutton!=nil) {
        if (self.type==OtherType) {
            [self updateInfofromKey:@"sex" andValue:[NSString stringWithFormat:@"%ld",self.selectbutton.tag-1000]];
        }
        else
        {
            [DLUser share].sex=[NSString stringWithFormat:@"%ld",self.selectbutton.tag-1000];
