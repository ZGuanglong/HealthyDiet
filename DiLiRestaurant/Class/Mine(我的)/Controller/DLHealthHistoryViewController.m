...
@interface DLHealthHistoryViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(weak,nonatomic)UITableView *tableView;

@end

@implementation DLHealthHistoryViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title=@"健康史";
    self.baseValue=[DLUserInfo getUserDetail].chronicDiseaseCode;
    self.titleLabel.text=@"请选择您的历史健康状况";
...
- (void)getData{
    ...
    dic.transCode=@"M0038";
    dic.type=@"findAll";
    [dic setObject:@24 forKey:@"dictType"];
...
- (void)done
{
    ...
    if (self.type==OtherType) {
       [self updateInfofromKey:@"chronicDiseaseCode" andValue:[selectArray componentsJoinedByString:@","]];
    }
    else
    {
        [DLUser share].chronicDiseaseCode=[selectArray componentsJoinedByString:@","];
        NSString *classStr=[[DLUser share].sex isEqualToString:@"2"]?@"DLSpecialWomanViewController":@"DLOtherDemandViewController";
        DLMineBaseViewController *baseVC=[[NSClassFromString(classStr)alloc]init];
