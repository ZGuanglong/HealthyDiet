...
@property(copy,nonatomic)NSString *otherName;//其他需求
...
- (NSMutableArray *)basedetailArray
{
    if (!_basedetailArray) {
        _basedetailArray=[NSMutableArray arrayWithArray:@[
            ...
            [DLUserInfo getUser].homeProvinceName,
            [DLUserInfo getUserDetail].currentProvinceName,
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.otherName=[DLUserInfo getUserDetail].otherReqName;
...
- (void)changeSex{
    if ([DLUserInfo getUser].sex.integerValue!=1&&[Globel getAge:[DLUserInfo getUser].birthday]>9&&[Globel getAge:[DLUserInfo getUser].birthday]<50) {
        [self changeState];
    }
    else
        [self.ChangeableArray removeAllObjects];
}
- (void)changeState{
    [self.ChangeableArray removeAllObjects];
    [self.ChangeableArray addObjectsFromArray:@[[NSMutableArray arrayWithArray:@[@"mine_special",@"特殊人群",[DLUserInfo getUserDetail].specialCrowdName,@"DLSpecialWomanViewController"]]]];
    if (strIsEmpty([DLUserInfo getUserDetail].specialCrowdName)) {
        NSString *str=strIsEmpty([DLUserInfo getUserDetail].periodStartTime)?@"":[NSString stringWithFormat:@"%@至%@",[DLUserInfo getUserDetail].periodStartTime,[DLUserInfo getUserDetail].periodEndTime];
        [self.ChangeableArray addObjectsFromArray:@[[NSMutableArray arrayWithArray:@[@"mine_slq",@"生理期",str,@"DLPhysiologyChioceViewController"]]]];
    }
}
- (void)creatHeadView{
    ...
    [iconimageView sd_setImageWithURL:[NSURL URLWithString:[Globel getUserInfoIcon]] placeholderImage:[UIImage imageNamed:@"defaultHead"]];
    ...
    phoneLabel.text=[NSString stringWithFormat:@"%@:\n\n%@",strIsEmpty([DLUserInfo getUser].mobileNum)?@"昵称":@"手机号",strIsEmpty([DLUserInfo getUser].mobileNum)?[DLUserInfo getUser].userName:[DLUserInfo getUser].mobileNum];
...
{
    if ([DLUserInfo getUser].sex.integerValue!=1){
        return 3;
    }
    return 2;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!indexPath.section) {
        ...
        baseVC.selectFinish=^(NSString *value){
            ...
            [unself.tableView reloadSections:[NSIndexSet indexSetWithIndex:2] withRowAnimation:UITableViewRowAnimationRight];
        };
        [self.navigationController pushViewController:baseVC animated:YES];
    }
    else{
        DLMineBaseViewController *baseVC=[[NSClassFromString(self.ChangeableArray[indexPath.row][3]) alloc]init];
        baseVC.type=OtherType;
        baseVC.selectFinish=^(NSString *value){
            [unself.ChangeableArray[indexPath.row] replaceObjectAtIndex:2 withObject:value];
            if (indexPath.row==0) {
                [unself changeState];
            }
            [unself.tableView reloadSections:[NSIndexSet indexSetWithIndex:1] withRowAnimation:UITableViewRowAnimationRight];
        };
        [self.navigationController pushViewController:baseVC animated:YES];
    }
}
