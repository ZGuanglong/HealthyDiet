...
-(instancetype)initWithMixNuber:(NSInteger)mixNuber maxNuber:(NSInteger)maxNuber showType:(rulerViewShowType)showType rulerMultiple:(NSInteger)rulerMultiple{
    ...
    if (self) {
        ...
        if (_showType==rulerViewshowHorizontalType) {//如果水平放置
            _rulerImage=[UIImage imageNamed:@"rulerscale_horizontal"];
            self.contentSize=CGSizeMake(RuleWidth*( BTminus(_maxNuber, _mixNuber)/rulerMultiple+1), self.frame.size.height);
        }else{
...
-(void)setUpRuleImageView{
    ...
    for (NSInteger i=0; i<=BTminus(_maxNuber,_mixNuber)/_rulerMultiple ; i++) {
        UIImageView *rulerImageView=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, RuleWidth, 40)];
        rulerImageView.image=self.rulerImage;
        ...
        rulerLable.textColor=[UIColor whiteColor];
...
-(void)layoutSubviews{
    ...
    for (UIView *sub in self.subviews) {
        CGPoint centerPoint;
        CGFloat rulerImageViewW = 0.0;
        CGFloat rulerImageViewH = 0.0;
        if ([sub isKindOfClass:[UIImageView class]]) {
            ...
            if (_showType==rulerViewshowHorizontalType) {
                rulerLableW=60;
                rulerLableH=20;
                rulerLableX =centerPoint.x-rulerLableW/2-RuleWidth/2;
