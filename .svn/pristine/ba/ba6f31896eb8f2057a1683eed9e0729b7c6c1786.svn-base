...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (!indexPath.section) {
        ...
        switch (indexPath.row) {
            case 0:
                cell.textLabel.text=[NSString stringWithFormat:@"主料:   %@",self.detailfoodInfo.mainFoodName];
                break;
            case 1:
                cell.textLabel.text=[NSString stringWithFormat:@"调料:   %@",self.detailfoodInfo.seasoningsName];
                break;
            case 2:
                cell.textLabel.text=[NSString stringWithFormat:@"食用油:  %@",self.detailfoodInfo.oilName];
            default:
                break;
        }
        return cell;
...
- (void)getdata{
    ...
    dic.transCode=@"M0067";
    dic.type=@"findDetail";
    [dic setObject:self.foodsCode forKey:@"dishesCode"];
    [[GLNetWorkManager shareManager]requestType:POST andURL:REQUESTURL andServers:GET_WORK_TYPE andParmas:dic andComplition:^(id response, BOOL isuccess) {
        if (isuccess&&[response[@"code"] isEqualToString:SUCCESSCODE]) {
            ...
            [_imageView sd_setImageWithURL:[NSURL URLWithString:self.detailfoodInfo.imagesURL] placeholderImage:[UIImage imageNamed:@"food_detail_default"]];
            self.nameLabel.text=[NSString stringWithFormat:@"    %@",self.detailfoodInfo.dishesName];
