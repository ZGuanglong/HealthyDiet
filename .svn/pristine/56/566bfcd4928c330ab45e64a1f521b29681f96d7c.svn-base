...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *leftItem = [UIBarButtonItem itemWithTargat:self action:@selector(backClick) image:@"navigationbar_back" highImage:@"navigationbar_back_highlighted"];
...
- (void)registBack
{
    [_webViewBridge registerHandler:@"backClick" handler:^(id data, WVJBResponseCallback responseCallback) {
        MyLog(@"返回");
        [self backClick];
    }];
}
